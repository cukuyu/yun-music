...
<div class="top-list">
    <div>
        ...
        <div 
        v-for="(list,listIndex) in officialList"
        :key="list.id"
        class="mbottom-30">
            ...
            <ul class="mleft-30">
                <li
                v-for="(music, index) in list.tracks?.slice(0,5)"
                :key="music.id"
                class="default-cursor stripe-list-item"
                @dblclick="playMusic(list.tracks,index)"
                >
...
const officialList = ref<musicListInfo[]>()
const globalList = ref<imgList[]>()
...
const getTopLists = async()=>{
    ...
    officialList.value?.forEach( async (item)=>{
        const tracksRes = await getPlayListDetail(item.id as string)
        if(tracksRes.code!=200) return
        item.tracks = tracksRes.playlist.tracks
    })
}

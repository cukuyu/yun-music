<template>
    <div class="artist-detail">
        <el-skeleton style="width: 60%; margin:20px 0 0 30px;" :loading="!artistdetail" animated>
            <template #template>
        ...
        <TabMenu :menuList="[{ name: '专辑' }, { name: 'MV' }, { name: '歌手详情' }, { name: '相似歌手' }]" @menuClick="handleClick"
            mode="menu" />
        <!-- 热门50首歌 -->
        <div v-show="tabIndex == 0" v-loading="!topList||topList.length==0">
            <ArtistSongList :list="topList" class="mbottom-40">
            </ArtistSongList>
            <ArtistSongList v-for="(item, index) in albumList" :list="item.songs" :album="item.album" class="mbottom-40">
            </ArtistSongList>
        </div>
        <!-- 歌手MV -->
        ...
        <!-- 歌手简介 -->
        <div class="artist-detail-intro" v-show="tabIndex == 2" v-loading="!introList || introList.length==0">
            <div v-for="item in introList" class="mbottom-40 artist-intro">
                <div>
                    <h1 class="font-14 font-bold">{{ item.ti }}</h1>
                    <p class="font-14 cl2-color" v-for="text in item.txt">
                        {{ text }}
                    </p>
                </div>
            </div>
        ...
        <div v-show="tabIndex == 3">
            <ImgList :list="sameList" type="artist" @clickImg="handClickImg">
...
import {
    getArtistDetail, getArtistTop, getArtistAlbum,
    getArtistMv, getArtistSame, getIntro,
} from '@/api/api_artist'
...
let topList = shallowRef<musicInfo[]>([])
let albumList = ref<albumsInfo[]>([])
let mvList = shallowRef<artistMv[]>([])
let introList = shallowRef<artistIntro[]>([])
let sameList = shallowRef<imgList[]>([])

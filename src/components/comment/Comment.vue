<template>
    <div class="comment-edit-dialog-wrap" v-if="editVisiable && defaultEdit">
        <div class="comment-edit-dialog">
            <CommentEdit @closeDig='showEditComment' :commentInfo="commentInfo" :name="editName"
                :defaultEdit="props.defaultEdit"></CommentEdit>
        </div>
    </div>
    <div class="comment-edit-dialog" v-if="!defaultEdit">
        <CommentEdit :commentInfo="commentInfo" :name="editName" :defaultEdit="props.defaultEdit"></CommentEdit>
    </div>
    <div class="comment" v-loading="commentLoading">
        <div class="hot-wrap mtop-20" v-if="hotList?.length !== 0">
            <div class="font-16 font-bold">全部评论({{ total }})</div>
            <CommentItem v-for="item in hotList" :key="item.commentId" :item="item" @like="toLikeComment"
                @editComment="editComment"></CommentItem>
            <div class="more-btn-wrap mtop-20">
                <button class="btn btn-white">更多精彩评论></button>
            </div>
        </div>
        <div class="new-wrap mtop-20" v-if="newList?.length !== 0" ref="newListRef">
            <div class="font-16 font-bold">最新评论({{ total }})</div>
            <CommentItem v-for="item in newList" :key="item.commentId" :item="item" @like="toLikeComment"
                @editComment="editComment"></CommentItem>
        </div>
        <div class="comment-pagination" v-show="total > 0">
            <el-pagination :current-page="currentPage" @current-change="handleCurrentChange" :page-size="query.limit"
                :pager-count="9" layout="prev, pager, next" :total="total" background>
            </el-pagination>
        </div>
    </div>
...
const props = defineProps({
    ...
    // true: 隐藏编辑评论 点击弹出， false: 直接显示编辑评论
    defaultEdit: {
        type: Boolean,
        default: false
    }
